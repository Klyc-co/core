...
serve(async (req) => {
  ...
  try {
    ...
    // Check if already connected as CRM
    ...
    // Build OAuth URL
    const redirectUri = `${supabaseUrl}/functions/v1/monday-crm-oauth-callback`;
    const state = btoa(JSON.stringify({ user_id: user.id, display_name: displayName || "Monday CRM" }));
    ...
    console.error("Error generating Monday CRM auth URL:", error);
