...
serve(async (req) => {
  ...
  if (error) {
    console.error("Tumblr OAuth error:", error);
  ...
  try {
    ...
    console.log("Processing Tumblr callback for user:", userId);

    const TUMBLR_CONSUMER_KEY = Deno.env.get("TUMBLR_CONSUMER_KEY")!;
    const TUMBLR_CONSUMER_SECRET = Deno.env.get("TUMBLR_CONSUMER_SECRET")!;
    ...
    console.log("Tumblr token exchange status:", tokenResponse.status);

    if (!tokenResponse.ok || !tokenData.access_token) {
      console.error("Tumblr token exchange failed:", tokenData);
    ...
    console.log("Tumblr user info status:", userResponse.status);

    let tumblrUsername = null;

    if (userResponse.ok && userInfoData.response?.user) {
      tumblrUsername = userInfoData.response.user.name;
      console.log("Connected Tumblr user:", tumblrUsername);
    }
    ...
    const { error: upsertError } = await supabase
      ...
      .upsert({
        ...
        platform_user_id: tumblrUsername,
        platform_username: tumblrUsername,
    ...
    if (upsertError) {
      console.error("Failed to store Tumblr connection:", upsertError);
      throw new Error("Failed to save Tumblr connection");
    }

    console.log("Tumblr connection saved successfully");
    return Response.redirect(`${frontendUrl}/profile/import?success=tumblr`);

  } catch (err) {
    console.error("Tumblr callback error:", err);
