  ...
  try {
    ...
    if (body.action === "health") {
      return new Response(JSON.stringify({
        status: "ok", version: AGENT_VERSION, permitted_lanes: [...PERMITTED_LANES],
    ...
    const platforms = body["μp"] || body.platforms || "";
    ...
    if (!isPeerCall) {
      if (lane === "strategy") {
        ...
        if (researchResult?.output) {
          peerContext.research = typeof researchResult.output === "string"
            ? researchResult.output : JSON.stringify(researchResult.output);
      ...
      if (lane === "performance") {
        ...
        if (strategyResult?.output) {
          peerContext.strategy = typeof strategyResult.output === "string"
            ? strategyResult.output : JSON.stringify(strategyResult.output);
    ...
    // ── AI call ───────────────────────────────────────────────────────────────
    const { text, tokensIn, tokensOut } = await callClaude(LANE_PROMPTS[lane], userPrompt);
    ...
    return new Response(JSON.stringify({
      lane, "δi": AGENT_ID, output: parsedOutput,
      peers_consulted: peersConsulted, is_peer_call: isPeerCall,
      version: AGENT_VERSION, elapsed_ms: elapsed,
    }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
