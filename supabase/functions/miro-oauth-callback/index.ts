...
serve(async (req) => {
  try {
    ...
    if (!MIRO_CLIENT_ID || !MIRO_CLIENT_SECRET) {
      return new Response("Miro credentials not configured", { status: 500 });
    }

    const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
    const redirectUri = `${SUPABASE_URL}/functions/v1/miro-oauth-callback`;
    ...
    if (!tokenResponse.ok) {
      ...
      console.error("Miro token exchange failed:", error);
    ...
    // Fetch user info from Miro
    let platformUserId = null;
    let platformUsername = null;
    try {
      const meResponse = await fetch("https://api.miro.com/v1/oauth-token", {
        headers: { Authorization: `Bearer ${tokens.access_token}` },
      });
      if (meResponse.ok) {
        const me = await meResponse.json();
        platformUserId = me.user?.id || null;
        platformUsername = me.user?.name || null;
      }
    } catch (e) {
      console.warn("Failed to fetch Miro user info:", e);
    }
    ...
    if (insertError) {
      console.error("Failed to save Miro connection:", insertError);
    ...
    return new Response(null, {
      status: 302,
      headers: {
        Location: `${frontendUrl}/profile/import?success=miro`,
      },
    });
  } catch (error: unknown) {
