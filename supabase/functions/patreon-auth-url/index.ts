...
serve(async (req) => {
  ...
  try {
    ...
    if (!PATREON_CLIENT_ID) {
      return new Response(
        JSON.stringify({ error: "Patreon not configured" }),
    ...
    // Patreon OAuth scopes for reading campaign data, members, and posts
    const scopes = [
      "identity",
      "identity[email]",
      "campaigns",
    ...
    console.log("Generated Patreon auth URL for user:", userId);
    ...
    console.error("Patreon auth URL error:", err);
    return new Response(
