...
const PLATFORM_SPECS: Record<string, {
  ...
  tiktok: {
    formats: ["Vertical Video (9:16)", "15-60s optimal"],
    maxChars: 2200,
    hashtagRange: [4, 7],
    ...
    genZUsage: "48% daily usage, #1 product discovery platform",
    toneGuide: "Casual, authentic, trend-aware, slightly chaotic energy OK",
    aspectRatios: ["9:16"],
  },
  instagram: {
    formats: ["Reels (9:16)", "Carousel (1:1)", "Stories (9:16)"],
    maxChars: 2200,
    hashtagRange: [5, 10],
  ...
  facebook: {
    ...
    genZUsage: "Community groups, marketplace, events",
...
interface PlatformPackage {
  ...
  adapted_cta: string;
  hashtags: string[];
  posting_time: { primary: string; secondary: string; timezone_note: string };
...
For EACH platform, return a JSON object with:
{
  "platform": "platform_name",
  "format_recommendation": "Best format for this content on this platform",
  "adapted_headline": "Platform-native headline",
  "adapted_body": "Body text adapted to platform tone/length",
  "adapted_hook": "Platform-specific hook (first line or first 3s concept)",
  "adapted_cta": "Platform-appropriate CTA",
  "hashtags": ["platform-optimized", "hashtags"],
  "platform_notes": "Specific tactical notes for this platform",
  "algorithm_tip": "How to maximize algorithm performance"
}

RULES:
- Preserve the CORE MESSAGE across all platforms
...
Return ONLY a JSON array of platform packages.`;
    ...
    // Enrich with specs & timing
    const packages: PlatformPackage[] = parsed.map((pkg: any) => {
      ...
      return {
        platform: p,
        format_recommendation: pkg.format_recommendation || spec.formats[0],
        aspect_ratios: spec.aspectRatios,
        adapted_headline: pkg.adapted_headline || "",
        adapted_body: bodyText,
        adapted_hook: pkg.adapted_hook || "",
        adapted_cta: pkg.adapted_cta || "",
        hashtags: Array.isArray(pkg.hashtags) ? pkg.hashtags : [],
        posting_time: timing,
        platform_notes: pkg.platform_notes || "",
