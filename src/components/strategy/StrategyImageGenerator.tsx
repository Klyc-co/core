...
import { Sparkles, Loader2, ImagePlus, Send, Check, Wand2 } from "lucide-react";
...
interface StrategyImageGeneratorProps {
  platform: string;
  /** Optional caption/script context to seed the prompt */
  context?: string | null;
}

export default function StrategyImageGenerator({
  platform,
  context,
}: StrategyImageGeneratorProps) {
  ...
  const seedPrompt = () => {
    if (context && context.trim()) {
      setPrompt(
        `Visual for ${platform}: ${context.slice(0, 400)}. High-quality, on-brand, scroll-stopping.`
      );
    } else {
      setPrompt(`A scroll-stopping ${platform} visual, on-brand, high quality.`);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    if (trimmed.length < 3) {
      toast({
        ...
        description: "Describe what image you want generated.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: e?.message || "Could not generate image.",
        variant: "destructive",
      });
    } finally {
      setGenerating(false);
  ...
  const handleSaveToBrand = async () => {
    ...
    try {
      ...
      toast({
        title: "Saved to Brand Assets",
        description: "You can find it in your brand library.",
      });
    } catch (e: any) {
      toast({
        title: "Save failed",
    ...
    try {
      ...
      if (pushMode === "existing") {
        ...
        const newImagePrompt =
          [existing?.image_prompt, imageUrl].filter(Boolean).join("\n");
        const newTags = Array.from(
          new Set([...(existing?.tags || []), `image:${imageUrl}`])
        );
        ...
        toast({
          ...
          description: "Added to the selected campaign draft.",
        });
      } else {
        ...
        toast({
          ...
          description: `"${name}" was created with this image.`,
        });
      }
      setPushOpen(false);
      setNewCampaignName("");
      ...
      toast({
        title: "Push failed",
        description: e?.message || "Could not push image.",
        variant: "destructive",
      });
    } finally {
      setPushing(false);
  ...
  return (
    <Card className="border-primary/20">
      <CardContent className="pt-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-foreground">
              Generate Image for {platform}
            </span>
          </div>
          {context && (
            <Button variant="ghost" size="sm" onClick={seedPrompt} className="text-xs">
        ...
        <div>
          <Label htmlFor={`img-prompt-${platform}`} className="text-xs text-muted-foreground">
            Image prompt
          </Label>
          <Textarea
            id={`img-prompt-${platform}`}
            placeholder={`Describe the image you want for ${platform}...`}
            value={prompt}
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <Button
                ...
                disabled={savingBrand || savedToBrand}
              >
                {savingBrand ? (
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                pushing ||
                (pushMode === "existing" && (!selectedCampaign || campaigns.length === 0))
              }
            >
              {pushing ? (
                <>
