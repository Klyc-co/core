 ...
 import { Upload, FolderOpen, X, Loader2, ImageIcon } from "lucide-react";
 ...
 interface ImageSourcePickerProps {
   onImageSelected: (imageUrl: string, imageName: string) => void;
 ...
 const ImageSourcePicker = ({
   ...
   onClearImage,
   label = "Select Image",
   description = "Choose an image from your device, Google Drive, or Klyc library"
   ...
   const handleGoogleDriveSelect = async (files: Array<{ id: string; name: string; path: string; thumbnailUrl?: string }>) => {
     ...
     if (file.thumbnailUrl) {
       onImageSelected(file.thumbnailUrl, file.name);
       toast({
       ...
       toast({
        ...
        description: "Could not load this image. Please try another one.",
         variant: "destructive",
       });
     }
   ...
   const handleLibrarySelect = (asset: LibraryAsset) => {
     onImageSelected(asset.url, asset.name);
