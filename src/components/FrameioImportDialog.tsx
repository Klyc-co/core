...
interface FrameioImportDialogProps {
  open: boolean;
...
const ACCEPTED_EXTENSIONS = ".mp4,.mov,.avi,.webm,.mkv,.mxf,.png,.jpg,.jpeg,.tiff,.webp,.gif,.psd,.pdf";

const FrameioImportDialog = ({ open, onOpenChange, userId }: FrameioImportDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} file${successCount > 1 ? "s" : ""} from Frame.io`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from Frame.io</DialogTitle>
          <DialogDescription>
            Download your approved assets from Frame.io, then upload them here to add to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">
            MP4, MOV, MXF, PNG, JPG, TIFF, PSD — up to 20 files
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          ...
          <p>• In Frame.io, select your asset and click <strong>Download</strong></p>
          <p>• For approved cuts: download the final version from the review page</p>
          <p>• You can also export stills or thumbnails from the player</p>
        </div>
...
export default FrameioImportDialog;
