...
const BlueprintGrid = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Grid lines
      ctx.strokeStyle = "rgba(107, 141, 227, 0.06)";
      ctx.lineWidth = 0.5;
      ...
      nodes.forEach((node) => {
        ...
        glow.addColorStop(0, `${node.color}${Math.round(pulse * 30).toString(16).padStart(2, "0")}`);
...
const LandingInfrastructure = () => {
  return (
    <section className="relative py-24 sm:py-32 lg:py-40 px-4 sm:px-6 bg-[#08080c] overflow-hidden">
      <BlueprintGrid />

      <div className="relative z-10 max-w-5xl mx-auto">
        <AnimateOnScroll>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight leading-tight mb-4">
            <span className="text-white">Stop Hiring Output.</span>
            <br />
        ...
        <AnimateOnScroll delay={100}>
          <p className="text-base sm:text-lg text-white/40 max-w-2xl mb-16 sm:mb-20 leading-relaxed font-light">
        ...
        <div className="grid md:grid-cols-3 gap-px rounded-xl overflow-hidden">
          {layers.map((layer, i) => (
            <AnimateOnScroll key={i} delay={150 + i * 100}>
              <div className="bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] p-8 sm:p-10 h-full flex flex-col">
                <layer.icon
