...
interface DescriptImportDialogProps {
  open: boolean;
...
const ACCEPTED_EXTENSIONS = ".mp4,.mov,.avi,.webm,.mp3,.wav,.m4a,.ogg,.png,.jpg,.jpeg,.webp,.gif";

const DescriptImportDialog = ({ open, onOpenChange, userId }: DescriptImportDialogProps) => {
    ...
    try {
      ...
      for (const sf of selectedFiles) {
        try {
          ...
          const assetType = sf.file.type.startsWith("video/")
            ? "video"
            : sf.file.type.startsWith("audio/")
            ? "audio"
            : "image";
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} file${successCount > 1 ? "s" : ""} from Descript`);
  ...
  const getFileIcon = (file: File) => {
    ...
    if (file.type.startsWith("audio/")) return <FileAudio className="w-5 h-5 text-muted-foreground" />;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from Descript</DialogTitle>
          <DialogDescription>
            Export your videos, audio, or stills from Descript, then upload them here to add to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">
            MP4, MOV, MP3, WAV, PNG, JPG — up to 20 files
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          ...
          <p>• In Descript, click <strong>Publish → Export</strong></p>
          <p>• Choose MP4 for video or MP3/WAV for audio-only</p>
          <p>• For clips: right-click a scene and select <strong>Export selection</strong></p>
        </div>
...
export default DescriptImportDialog;
