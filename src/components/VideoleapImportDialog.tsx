...
interface VideoleapImportDialogProps {
  open: boolean;
...
const VideoleapImportDialog = ({ open, onOpenChange, userId }: VideoleapImportDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} file${successCount > 1 ? "s" : ""} from Videoleap`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from Videoleap</DialogTitle>
          <DialogDescription>
            Export your videos or images from Videoleap, then upload them here to add to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          ...
          <p>• Tap <strong>Export</strong> in Videoleap to save your video to your device</p>
...
export default VideoleapImportDialog;
