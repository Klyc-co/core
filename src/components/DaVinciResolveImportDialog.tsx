...
interface DaVinciResolveImportDialogProps {
  open: boolean;
...
const ACCEPTED_TYPES = [
  ...
  "application/xml", "text/xml", // DaVinci Resolve project XML exports
];

const ACCEPTED_EXTENSIONS = ".mp4,.mov,.avi,.webm,.mkv,.png,.jpg,.jpeg,.tiff,.webp,.xml,.drp";

const DaVinciResolveImportDialog = ({ open, onOpenChange, userId }: DaVinciResolveImportDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} file${successCount > 1 ? "s" : ""} from DaVinci Resolve`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from DaVinci Resolve</DialogTitle>
          <DialogDescription>
            Export your videos or stills from DaVinci Resolve, then upload them here to add to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">
            MP4, MOV, AVI, PNG, JPG, TIFF, WebM — up to 20 files
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          ...
          <p>• Use <strong>File → Deliver</strong> in DaVinci Resolve to export</p>
          <p>• For social media: H.264 MP4 at 1080p is recommended</p>
          <p>• For stills: Export as PNG for best quality</p>
        </div>
...
export default DaVinciResolveImportDialog;
