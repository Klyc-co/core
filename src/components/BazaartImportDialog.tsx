...
interface BazaartImportDialogProps {
  open: boolean;
...
const ACCEPTED_EXTENSIONS = ".png,.jpg,.jpeg,.webp,.gif,.heic";

const BazaartImportDialog = ({ open, onOpenChange, userId }: BazaartImportDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} image${successCount > 1 ? "s" : ""} from Bazaart`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from Bazaart</DialogTitle>
          <DialogDescription>
            Export your designs from Bazaart, then upload them here to add to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <p className="text-sm font-medium text-foreground">
            Drop exported images here or click to browse
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            PNG, JPG, WebP, GIF — up to 20 files
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          ...
          <p>• Tap <strong>Save</strong> in Bazaart to export your design to your device</p>
          <p>• For best quality: use PNG format for graphics with transparency</p>
          <p>• JPG works great for photos and social media posts</p>
        </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={handleUpload} disabled={selectedFiles.length === 0 || uploading}>
            {uploading ? (
              ...
              `Import ${selectedFiles.length} image${selectedFiles.length !== 1 ? "s" : ""}`
            )}
          </Button>
        </div>
      </DialogContent>
...
export default BazaartImportDialog;
