...
interface VeedImportDialogProps {
  open: boolean;
...
const VeedImportDialog = ({ open, onOpenChange, userId }: VeedImportDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} file${successCount > 1 ? "s" : ""} from VEED.io`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from VEED.io</DialogTitle>
          <DialogDescription>
            Export your videos or images from VEED.io, then upload them here to add to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          ...
          <p>• Click <strong>Export</strong> in VEED.io to download your video</p>
          <p>• For best quality: use 1080p or 4K resolution</p>
          <p>• You can also export subtitled videos or thumbnails</p>
        </div>
...
export default VeedImportDialog;
