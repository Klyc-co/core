...
interface OBSStudioImportDialogProps {
  open: boolean;
...
const ACCEPTED_EXTENSIONS = ".mp4,.mkv,.mov,.flv,.avi,.webm,.png,.jpg,.jpeg,.webp,.gif";

const OBSStudioImportDialog = ({ open, onOpenChange, userId }: OBSStudioImportDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Imported ${successCount} file${successCount > 1 ? "s" : ""} from OBS Studio`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Import from OBS Studio</DialogTitle>
          <DialogDescription>
            Upload your OBS recordings and screenshots to add them to your library.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <p className="text-sm font-medium text-foreground">
            Drop recorded files here or click to browse
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            MP4, MKV, MOV, FLV, AVI, WebM, PNG, JPG — up to 20 files
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          <p className="font-medium text-foreground">Finding your recordings:</p>
          <p>• Default location: <strong>Videos</strong> folder on your computer</p>
          <p>• Check OBS Settings → Output → Recording Path</p>
          <p>• Screenshots can be taken via OBS hotkeys or the Screenshot button</p>
        </div>
...
export default OBSStudioImportDialog;
