...
import {
  FileText, ChevronDown, AlertTriangle, CheckCircle2,
  Brain, Target, Cpu, Lightbulb, ShieldAlert, Globe,
  Building2, Users, Crosshair, Package, Shield, Zap,
} from "lucide-react";
import type { NormalizerReport, CampaignBrief, CustomerContext, OrchestratorHints, LearningHooks } from "@/types/normalizer-report";
...
const FieldRow = ({ label, value, missing }: { label: string; value: string | null | undefined; missing?: boolean }) => (
...
const ConfidenceBar = ({ score }: { score: number | null }) => {
  const s = score ?? 0;
  const color = s >= 70 ? "bg-primary" : s >= 40 ? "bg-yellow-500" : "bg-destructive";
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted-foreground">Confidence</span>
        <span className="text-xs font-mono font-semibold text-foreground">{s}%</span>
      </div>
...
const CoverageBar = ({ value }: { value: number }) => {
  const color = value >= 80 ? "bg-primary" : value >= 50 ? "bg-yellow-500" : "bg-destructive";
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted-foreground">Context Coverage</span>
...
const ContextSection = ({ data }: { data: CustomerContext }) => (
  <Section title="Customer Context" icon={<Brain className="w-3.5 h-3.5 text-primary" />}>
    <CoverageBar value={data.contextCoverage} />
    <div className="flex items-center justify-between text-xs pb-1">
      <span className="text-muted-foreground">Sources Loaded</span>
      <span className="font-mono text-foreground">{data.sourceCount}</span>
    </div>
    {data.lastUpdated && (
      <div className="flex items-center justify-between text-xs pb-1">
...
const HintsSection = ({ data }: { data: OrchestratorHints }) => {
  ...
  return (
    <Section title="Orchestrator Hints" icon={<Cpu className="w-3.5 h-3.5 text-primary" />}>
      ...
      {data.missingCriticalInputs.length > 0 && (
        <div className="rounded-md border border-destructive/20 bg-destructive/5 p-2 space-y-1">
          <span className="flex items-center gap-1.5 text-[10px] font-medium text-destructive uppercase tracking-wider">
  ...
  <Section title="Learning Hooks" icon={<Lightbulb className="w-3.5 h-3.5 text-primary" />}>
    ...
    {data.recommendedNextUpdate && (
      <div className="flex items-center justify-between text-xs pt-1">
        <span className="text-muted-foreground">Recommended Next Update</span>
        <span className="text-foreground">{data.recommendedNextUpdate}</span>
      </div>
    )}
  </Section>
);
...
export default function NormalizerReportPanel({ report }: Props) {
  ...
  const missingCount =
    report.orchestratorHints.missingCriticalInputs.length +
    report.learningHooks.missingInputs.length;
