...
const NewCampaign = () => {
  ...
  // Re-fetch connections when user returns to this tab after OAuth in new window
  useEffect(() => {
    const handleFocus = () => { if (user) fetchPlatformConnections(user.id); };
  ...
  const OAUTH_FUNCTION_MAP: Record<string, { functionName: string; urlKey: string }> = {
    twitter: { functionName: "twitter-auth-url", urlKey: "authUrl" },
