...
const GenerateCampaignIdeas = () => {
  ...
  // Pre-seed reference image if navigated from Creative Studio
  useEffect(() => {
    const imageUrl = (location.state as { referenceImageUrl?: string } | null)?.referenceImageUrl;
    if (imageUrl) {
      setReferenceImages([{ url: imageUrl, name: "From Creative Studio" }]);
      // Clear state so back-nav doesn't re-seed
